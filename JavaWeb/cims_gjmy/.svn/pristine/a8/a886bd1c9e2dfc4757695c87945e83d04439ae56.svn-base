...
public class OperatorDAO extends C3P0BaseDAO {
    ...
    public List<Operator> findAll() throws ClassNotFoundException, SQLException {
        //????????
        List<Operator> operators = new ArrayList<Operator>(); //???????????null
        int i = 0;
        //1?????????? 2??????rbac?????
        Connection conn = super.getConnection();
        //3?????????????????? java.sql.Connection,????????????????java.sql.PreparedStatement????????SQL???
        ...
        while (rs.next()) {
			...
			i++;
			*/
            operators.add(new Operator(rs.getInt("id"), rs.getString("username"), rs.getString("password"), rs.getString("tel"),
