...
public class RoleDAO extends C3P0BaseDAO {
    ...
    public List<Role> findAll() throws ClassNotFoundException, SQLException {
        //????????
    	List<Role> roles = new ArrayList<Role>(); //???????????null
        //int i = 0;
        //1?????????? 2??????rbac?????
        Connection conn = super.getConnection();
        //3?????????????????? java.sql.Connection,????????????????java.sql.PreparedStatement????????SQL???
	...
	public List<Role> findAll(int currentPage, int pageSize) throws SQLException, ClassNotFoundException {
		List<Role> roles = new ArrayList<Role>(); 
	    Connection conn = super.getConnection();
	    PreparedStatement pstmt = conn.prepareStatement("select * from role order by id limit ?,?");
